...
def thread_count():
    ...
    return jsonify({"thread_count": count, "threads": thread_names})

def setup_connection(com_port):
    global global_connection, drone_thread
    ...
    drone_thread = Thread(target=listen_to_drones)
    drone_thread.start()

def setup_rtk_connection(com_port, baud_rate):
    global global_rtk_connection, rtk_thread
    if global_rtk_connection:
        global_rtk_connection.close()
...
def set_com_port():
    ...
    if global_connection:
        current_port = global_connection.port
        if current_port == com_port:
            return jsonify(success=True, message="Already connected to the specified COM port.")
...
def set_rtk_port():
    ...
    if global_rtk_connection:
        current_rtk_port = global_rtk_connection.port
        if current_rtk_port == rtk_com_port:
            return jsonify(success=True, message="Already connected to the specified RTK port.")
